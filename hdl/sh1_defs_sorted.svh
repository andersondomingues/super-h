typedef enum logic[9:0] { 
    DIRECT_REGISTER = 0,     // Rn, use content of register
    INDIRECT_REGISTER = 1,   // @Rn, use content of address in register
    INDIRECT_POST_INC = 2, // @Rn+, use content of address in register
    INDIRECT_PRE_DEC = 3,  // @-Rn, decrement register and use content of address in register
    INDIRECT_DISP = 4,     // @(disp, Rn), use content of address in register + displacement
    INDIRECT_INDEXED = 5, // @(Rm, Rn), use content of address in register Rm + Rn
    INDIRECT_GBR_DISP = 6, // @(disp, GBR), use content of address in GBR + displacement
    INDIRECT_GBR_INDEXED = 7, // @(Rm, GBR), use content of address in GBR + Rm
    PC_RELATIVE_INDEXED = 8, // @(disp, PC), use content of address in PC + displacement
    PC_RELATIVE = 9,        // @(disp, PC), use content of address in PC + displacement
    IMMEDIATE = 10          // #imm, use immediate value
} addressing_mode_e;
